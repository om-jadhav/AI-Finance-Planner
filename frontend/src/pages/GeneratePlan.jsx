...
function DynamicKeyValue({ data }) {
    ...
    return (<div className="dynamic-kv-list">
        {entries.map(([key, value]) => (<div className="dynamic-kv-row" key={key}> <span className="dynamic-kv-key">{formatLabel(key)}</span> <span className="dynamic-kv-value">{formatValue(value)}</span> </div>
        ))} </div>
    );
}
...
function DynamicCategoryList({ data }) {
    ...
    return (<div className="dynamic-category-list">
        {entries.map(([category, items]) => (<div className="dynamic-category" key={category}> <h4 className="dynamic-category-title">
            {formatLabel(category)} </h4>

            {Array.isArray(items) && items.length > 0 ? (
                <div className="instrument-grid">
                    {items.map((item, idx) => (
                        <div className="dynamic-instrument-card" key={idx}>
                            <DynamicKeyValue data={item} />
                        </div>
                    ))}
                </div>
            ) : (
...
export default function GeneratedPlan() {
    ...
    if (loading) {
        return (<div className="dashboard-layout"> <Sidebar active="plan" />
            ...
            <div className="dashboard-content profile-page-content">
                ...
                {!response && !generating && (
                    <div className="profile-card plan-empty-card">
                        ...
                        <p>
                            Generate a personalized financial plan using your income,
                            expenses, goals, investment horizon, and risk profile.
                        </p>

                        <button
                            className="primary-button plan-empty-button"
                            onClick={handleGenerate}
                        >
                            Generate My Plan
                        </button>
                ...
                {generating && (
                    <div className="profile-card plan-generating-card">
                        ...
                        <p>
                            Our AI is analyzing your financial profile, goals, risk
                ...
                {response && !generating && (
                    <div className="plan-sections">
                        ...
                        <section className="plan-card">
                            ...
                            <div className="plan-stat-row">
                                ...
                                <div className="plan-stat">
                                    ...
                                    <span className="plan-stat-value">
                                        {formatCurrency(
                                            response.risk_profile?.monthly_surplus
                                        )}
                                    </span>
                                </div>
                            </div>
                            ...
                            {response.risk_profile?.factor_breakdown && (
                                <>
                                    ...
                                    <DynamicKeyValue
                                        data={response.risk_profile.factor_breakdown}
                                    />
                                </>
                            )}
                        </section>
                        ...
                        <section className="plan-card">
                            ...
                            <div
                                className={`feasibility-badge ${response.feasibility?.goal_feasible
                                        ? "feasible"
                                        : "not-feasible"
                                    }`}
                            >
                                ...
                                <span>
                                    {response.feasibility?.goal_feasible
                                        ? "On track"
                                        : "Needs adjustment"}

                                    {response.feasibility?.status &&
                                        ` — ${response.feasibility.status}`}
                                </span>
                            </div>
                            ...
                            <div className="plan-stat-row plan-stat-row-large">
                                <div className="plan-stat">
                                    ...
                                    <span className="plan-stat-value">
                                        {formatCurrency(
                                            response.feasibility?.goal_amount
                                        )}
                                    </span>
                                </div>
                                ...
                                <div className="plan-stat">
                                    ...
                                    <span className="plan-stat-value">
                                        {formatCurrency(
                                            response.feasibility?.projected_value
                                        )}
                                    </span>
                                </div>
                                ...
                                <div className="plan-stat">
                                    ...
                                    <span className="plan-stat-value">
                                        {formatCurrency(
                                            response.feasibility?.investment_gap
                                        )}
                                    </span>
                                </div>
                                ...
                                <div className="plan-stat">
                                    ...
                                    <span className="plan-stat-value">
                                        {response.feasibility?.assumed_annual_return ?? "—"}%
                                    </span>
                                </div>
                        ...
                        <section className="plan-card">
                            ...
                            <DynamicKeyValue
                                data={response.portfolio?.allocation}
                            />
                            ...
                            <DynamicCategoryList
                                data={response.portfolio?.recommended_instruments}
                            />
                        </section>
                            ...
                            {Array.isArray(response.plan?.plans) &&
                                ...
                                <div className="plan-options-list">
                                    {response.plan.plans.map((p, idx) => (
                                        <div
                                            className="dynamic-instrument-card plan-option-card"
                                            key={idx}
                                        >
                                            <div className="plan-option-number">
                                                {idx + 1}
                                            </div>

                                            <DynamicKeyValue data={p} />
                                        </div>
                                    ))}
                                </div>
                            ) : (
                                <p className="plan-empty">
                                    No plan options returned.
                                </p>
                            )}
                        ...
                        {plan.createdAt && (
                            <p className="plan-generated-at">
                                ...
                                {response.metadata?.llm_used === false &&
                                    " · Generated using fallback logic"}
                            </p>
                        )}
                    </div>
                )}
